...
//	              }
	...
	@Override
	public void dataCart(Model model, String email) {
		model.addAttribute("email", email);

		Integer cartNum = this.getCartnum(email);
		if (cartNum == null) {
			this.cartInsert(email);
		}

		model.addAttribute("cartNum", cartNum);

		// cart에 있는 상품 정보 가져오기
		cartNum = this.getCartnum(email);
		List<ItemVO> ivo = this.detailCartItems(cartNum);
		List<Integer> sales = this.extractSales(ivo);
		model.addAttribute("itemList", ivo);

		List<Integer> itemNumList = this.detailCartItemstest(cartNum);
		...
		int memNum = this.getMembership(email);

		int pointRate = this.getPointRate(memNum);
		int deliveryCharge = this.deliveryCharge(memNum);
		model.addAttribute("deliveryCharge", deliveryCharge);
		model.addAttribute("pointRate", pointRate);

		List<Integer> extractCartItemCnts = this.extractCartItemCnts(cartNum);
		List<String> cartSizes = this.extractCartItemSize(cartNum);
		...
		List<Integer> totalPriceList = this.totalPriceList(saleList, extractCartItemCnts);
		model.addAttribute("totalPriceList", totalPriceList);

		int totalPrice = this.calculateTotalPrice(saleList, extractCartItemCnts);
		...
		List<Integer> itemRewardPoints = this.itemRewardPoints(saleList, pointRate, extractCartItemCnts);
