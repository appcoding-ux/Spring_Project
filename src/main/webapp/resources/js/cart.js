...
$(function () {
    ...
    // 전체상품삭제 버튼 클릭 시
    $("#allDeleteButton").click(function (e) {
        // cartNum 값을 가져오기
        ...
        $.ajax({
            ...
            success: function (response) {
                console.log(response);
                $(e.target).siblings("table").find("tbody").children("tr").remove();
                $(e.target).siblings("table").find("tbody").append("<td><div>장바구니에 담긴 상품이 없습니다.</div></td>");
                $(".totalPriceUpdate").text(0 + "원");
                $(".cart-won").text(0 + "원");
                $(".TotalPriceWithShippingUpdate").text(3000 + "원");
                $("#totalPrice").val(0);
                $("#TotalPriceWithShipping").val(3000);
            },
...
function selectOrder() {
    var frag = true;
    $(".itemCheckbox").each(function () { // 일단 모든 체크박스 순회
        if($(this).is(':checked')){ // 체크가 되어있는경우
            frag = false;
        }else {
            // 체크안되어 있는 경우
    ...
    if(frag){
        alert("현재 장바구니에 상품이 없어 주문으로 이동할 수 없습니다.");
    }else {
        $('#checkoutForm').submit();
    }
};
...
function updateTotalPrice() {
    ...
    let oriTotalPriceWithShipping = totalPrice + deliveryCharge;
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if(frag){
                    ...
                    $("#totalPrice").val(0);
                    $("#TotalPriceWithShipping").val(3000);
                    $(".selectNext").siblings("table").find("tbody").append("<td><div>장바구니에 담긴 상품이 없습니다.</div></td>");
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
                // 에러 처리
