		...
		<div class="info_box">
			<div class="info_left">
				<ul>
					...
					<li class="list_name"><span>${user.userName}</span> 
						<span>
							<c:choose>
								<c:when test="${user.memNum eq 0}">Loby 회원</c:when>
								<c:when test="${user.memNum eq 1}">Lounge 회원</c:when>
								<c:when test="${user.memNum eq 2}">Shard 회원</c:when>
							</c:choose>
						</span>
					</li>
					<li>
						<a href="/user/membership">등급 혜택보기</a><span class="bar"></span>
						<a href="/shard/userCheck">회원정보수정</a>
					</li>
				</ul>
			</div>
